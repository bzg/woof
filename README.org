...
There are 4 basic "triggers" for dealing with bugs and patches:

- =Confirmed= registers a new bug.
- =Fixed= marks a registered bug as fixed and un-register it.
- =[PATCH= in the email subject registers a new patch.
- =Applied= marks a patch as applied and un-register it.

=Applied=, =Confirmed= and =Fixed= should be placed at the very beginning of
a line in a *plain-text email*.  Beware that the match is
case-sensitive, so "applied", "CONFIRMED" and "FiXeD" won't work.

* Advanced usage: via mail headers
