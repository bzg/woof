...
4 triggers allows to deal with bugs and patches:

- =Confirmed= in the body of the email registers a new bug.
- =Fixed= in the body of the email marks a bug as fixed and un-registers it.
- =[PATCH= in the email subject registers a new patch.
- =Applied= in the body of the email marks a patch as applied and un-registers it.
