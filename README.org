...
All updates (for bugs, patches, help requests and new releases) can 
be triggered by using specific =X-Woof-*= mail headers.

[[file:woof.png]]

* Basic usage

4 triggers allows to manage bugs and patches:
...
- =[PATCH]= in the subject of a new email registers a new patch.
...
Series of patch with a subject containing e.g. =[PATCH 1/3]= are not
handled right now.
